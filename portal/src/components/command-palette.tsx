...
const ITENS: ItemBusca[] = [
  ...AVISOS.map((a) => ({
    ...
    tipo: "aviso" as const,
    titulo: a.titulo,
    secundaria: `${ROTULO_CATEGORIA[a.categoria] ?? a.categoria} · ${a.data}`,
    corpo: a.resumo,
    href: `/avisos`,
  })),
  ...DOCUMENTOS.map((d) => ({
    ...
    titulo: d.titulo,
    secundaria: `${d.tipo} · Protocolo ${d.protocolo}`,
    corpo: `Situação: ${d.situacao} · Previsão: ${d.previsao}`,
    href: "/documentos",
  })),
  ...PROJETOS.map((p) => ({
    id: `proj-${p.id}`,
    ...
    titulo: p.titulo,
    secundaria: `${p.autor} · ${p.eixo}`,
    corpo: `${p.vagas} vagas ofertadas · Situação: ${p.situacao}`,
    href: "/projetos",
  })),
];
