...
const feedData = [
  {
    ...
    categoria: 'matricula', publico: ['aluno'],
  ...
  },
  {
    ...
    categoria: 'cancelamento', publico: ['aluno', 'professor', 'servidor'],
  ...
  },
  {
    ...
    categoria: 'edital', publico: ['aluno'],
  ...
  },
  {
    ...
    categoria: 'evento', publico: ['aluno', 'professor'],
  ...
  },
  {
    ...
    categoria: 'calendario', publico: ['professor', 'servidor'],
  ...
  },
  {
    ...
    categoria: 'edital', publico: ['aluno', 'professor', 'servidor'],
...
function renderFeed() {
  ...
  const filtered = feedData.filter(item => {
    const matchPublico   = activoPublico === 'todos' || item.publico.includes(activoPublico);
    const matchCategoria = activaCategoria === 'todas' || item.categoria === activaCategoria;
  ...
  const tagClasses = {
    matricula: 'tag-matricula', evento: 'tag-evento',
    cancelamento: 'tag-cancelamento', edital: 'tag-edital',
    calendario: 'tag-calendario', geral: 'tag-geral'
  };
  const tagLabels = {
    matricula: 'Matrícula', evento: 'Evento',
    cancelamento: 'Cancelamento', edital: 'Edital',
    calendario: 'Calendário', geral: 'Geral'
  };
  const publicoLabel = { aluno: 'Aluno', professor: 'Professor', servidor: 'Servidor' };
...
document.getElementById('filterPublico').addEventListener('click', e => {
  ...
  if (!btn) return;
  document.querySelectorAll('#filterPublico .chip').forEach(c => c.classList.remove('active'));
...
document.getElementById('filterCategoria').addEventListener('click', e => {
  const btn = e.target.closest('.chip');
  if (!btn) return;
  document.querySelectorAll('#filterCategoria .chip').forEach(c => c.classList.remove('active'));
  btn.classList.add('active');
...
const TIPO_LABEL = { prova: 'Prova', prazo: 'Prazo', evento: 'Evento', feriado: 'Feriado' };
...
function renderCalendario() {
  ...
  for (let i = 1; i <= resto; i++) html += `<div class="cal-day other-month">${i}</div>`;

  grid.innerHTML = html;

  const eventosMes = calEventos.filter(e => e.ano === calAno && e.mes === calMes)
...
const docsData = [
  { cat: 'Formulários', icone: '📋', classe: 'doc-icon-form',   nome: 'Requerimento Geral',             desc: 'Solicitações diversas à secretaria acadêmica', href: '#' },
  { cat: 'Formulários', icone: '📋', classe: 'doc-icon-form',   nome: 'Aproveitamento de Disciplina',    desc: 'Dispensa de matéria por conhecimento prévio',   href: '#' },
  { cat: 'Formulários', icone: '📋', classe: 'doc-icon-form',   nome: 'Declaração de Vínculo',           desc: 'Documento para comprovar matrícula ativa',       href: '#' },
  { cat: 'Formulários', icone: '📋', classe: 'doc-icon-form',   nome: 'Trancamento de Matrícula',        desc: 'Suspensão temporária das atividades acadêmicas', href: '#' },
  { cat: 'Manuais e Regulamentos', icone: '📖', classe: 'doc-icon-manual', nome: 'Regulamento Acadêmico 2026',    desc: 'Normas gerais de ensino, avaliação e conduta',   href: '#' },
  { cat: 'Manuais e Regulamentos', icone: '📖', classe: 'doc-icon-manual', nome: 'Manual do Aluno Ingressante',   desc: 'Guia de orientação para calouros',               href: '#' },
  { cat: 'Manuais e Regulamentos', icone: '📖', classe: 'doc-icon-manual', nome: 'Manual de TCC',                 desc: 'Normas para elaboração de trabalho de conclusão', href: '#' },
  { cat: 'E-mails e Contatos', icone: '✉️', classe: 'doc-icon-email',  nome: 'Secretaria Acadêmica',          desc: 'secretaria@arcadia.edu.br',                      href: 'mailto:secretaria@arcadia.edu.br' },
  { cat: 'E-mails e Contatos', icone: '✉️', classe: 'doc-icon-email',  nome: 'Coordenação Geral',             desc: 'coordenacao@arcadia.edu.br',                     href: 'mailto:coordenacao@arcadia.edu.br' },
  { cat: 'E-mails e Contatos', icone: '✉️', classe: 'doc-icon-email',  nome: 'Ouvidoria',                     desc: 'ouvidoria@arcadia.edu.br',                       href: 'mailto:ouvidoria@arcadia.edu.br' },
  { cat: 'Bibliotecas Digitais', icone: '📚', classe: 'doc-icon-lib',   nome: 'Portal Periódicos CAPES',       desc: 'Acesso a artigos e revistas científicas',        href: 'https://www.periodicos.capes.gov.br' },
  { cat: 'Bibliotecas Digitais', icone: '📚', classe: 'doc-icon-lib',   nome: 'Biblioteca Virtual Pearson',    desc: 'E-books de diversas disciplinas',                href: '#' },
  { cat: 'Bibliotecas Digitais', icone: '📚', classe: 'doc-icon-lib',   nome: 'SciELO Brasil',                 desc: 'Biblioteca científica eletrônica online',         href: 'https://scielo.br' },
  { cat: 'Links Úteis', icone: '🔗', classe: 'doc-icon-link',  nome: 'Portal do Aluno',               desc: 'Notas, histórico, matrícula online',              href: '#' },
  { cat: 'Links Úteis', icone: '🔗', classe: 'doc-icon-link',  nome: 'Sistema de Frequência',         desc: 'Controle de presença e faltas',                   href: '#' },
  { cat: 'Links Úteis', icone: '🔗', classe: 'doc-icon-link',  nome: 'e-MEC',                         desc: 'Informações oficiais do MEC sobre o curso',       href: 'https://emec.mec.gov.br' },
];
...
function renderDocs(filtro = '') {
  ...
  const grupos = {};
  filtered.forEach(d => { (grupos[d.cat] = grupos[d.cat] || []).push(d); });
  ...
  grid.innerHTML = html || '<p style="padding:20px;color:var(--muted)">Nenhum resultado encontrado.</p>';
}

document.getElementById('docsSearch').addEventListener('input', e => renderDocs(e.target.value));
renderDocs();
